<?php 
    ...
    if($_SERVER['REQUEST_METHOD']==='POST'){ // Verifica si el formulario fue enviado
        ...
        if(!$imagen['name'] || $imagen['error']){//Si no hay imagen o si hay error
        ...
        if(empty($errores)){//empty es para si el arreglo esta vacio
            ...
            if($resultado){
                ...
                header('Location: /admin?resultado=1'); // Redirige al panel admin y usamos query string
            }
        }
    }
...
?>
    ...
    <main class="contenedor">
        ...
        <!-- Formulario -->
        <form class="formulario" method="POST" action="/admin/propiedades/crear.php" enctype="multipart/form-data"><!-- Es necesario para permitir subir archivos -->
            <fieldset>
